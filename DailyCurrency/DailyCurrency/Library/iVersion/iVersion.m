		...
		*/
		...
		if (showDetails && !self.visibleRemoteAlert)
		{
			...
			UIAlertView *alert = [[UIAlertView alloc] initWithTitle:title
												  ...
												  cancelButtonTitle:nil // self.ignoreButtonLabel, modified by RyuCJ 2013.1.29
...
- (BOOL)checkForNewVersionInBackground
{
	@synchronized (self)
	{
		@autoreleasepool
...
- (void)openAppPageInAppStore
{
	if (_displayAppUsingStorekitIfAvailable && [SKStoreProductViewController class])
	{
		if (self.verboseLogging)
		{
			NSLog(@"iVersion will attempt to open the StoreKit in-app product page using the following app store ID: %lu", (unsigned long)self.appStoreID);
		}
...
//		if (!rootViewController && [appDelegate respondsToSelector:@selector(window)])
		if (!rootViewController && [appDelegate valueForKey:@"window"] != nil)
		{
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex
{
	...
	else if (buttonIndex == 1) // == 2, modified by RyuCJ 2013.1.29
	{
		//remind later
